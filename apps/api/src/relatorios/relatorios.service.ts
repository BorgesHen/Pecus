...
/** Fallback final se a empresa não tiver rendimentoCarcacaPadrao (não deve ocorrer — tem default no schema). */
const RC_PADRAO = 52;
/** 1 Unidade Animal = 450 kg de peso vivo. */
const KG_POR_UA = 450;
/** Categorias de gasto que entram na conversão alimentar / custo de dieta. */
...
export class RelatoriosService {
  ...
  async dashboard(empresaId: string) {
    ...
    const [vencidos, proximos7Dias] = await Promise.all([
      this.prisma.eventoSanitario.count({
        where: { empresaId, proximaAplicacao: { not: null, lt: hoje } },
      }),
      this.prisma.eventoSanitario.count({
        where: { empresaId, proximaAplicacao: { gte: hoje, lte: em7Dias } },
      }),
    ]);
  ...
  async custoPorArroba(empresaId: string, loteId: string) {
    const [lote, empresa] = await Promise.all([
      ...
      this.prisma.empresa.findUnique({
        where: { id: empresaId },
        select: { rendimentoCarcacaPadrao: true },
      }),
    ]);
    ...
    // Arroba é peso de CARCAÇA, não peso vivo — por isso aplica o rendimento de carcaça.
  ...
  /**
   * Indicadores específicos do método de manejo atual do lote, calculados só
   * para a fase em andamento (não a vida toda do lote) — ex: custo da arroba
   * "da terminação", conversão alimentar da fase de confinamento, etc.
   */
  async indicadoresMetodo(empresaId: string, loteId: string) {
    const [lote, empresa] = await Promise.all([
      this.prisma.lote.findFirst({
        where: { id: loteId, empresaId },
        include: {
    ...
    if (!metodoManejo) {
      return {
        temMetodo: false as const,
        mensagem: 'Lote sem método de manejo definido — sem fórmulas específicas a aplicar.',
      };
    }
    ...
    const pesoInicialFase =
      faseInicio.getTime() === lote.dataAquisicao.getTime()
        ...
        : (lote.pesagens.filter((p) => p.data <= faseInicio).slice(-1)[0]?.pesoMedio ??
          pesagensNaFase[0]?.pesoMedio ??
          0);
    const pesoAtual = lote.pesagens[lote.pesagens.length - 1]?.pesoMedio ?? pesoInicialFase;

    const dias = Math.max(
      1,
      Math.round((Date.now() - faseInicio.getTime()) / (1000 * 60 * 60 * 24)),
    );
    ...
    const custoPorArrobaFase =
      arrobasProduzidasFase > 0 ? Number((custoTotalFase / arrobasProduzidasFase).toFixed(2)) : null;

    const gastosAlimentacao = gastosDaFase.filter((g) =>
      CATEGORIAS_ALIMENTACAO.includes(g.categoria as CategoriaGasto),
    );
    const custoAlimentacaoFase = gastosAlimentacao.reduce((acc, g) => acc + Number(g.valor), 0);
    const kgAlimentacaoFase = gastosAlimentacao.reduce(
      (acc, g) => acc + (g.unidade?.toLowerCase() === 'kg' ? (g.quantidade ?? 0) : 0),
      0,
    );
    const conversaoAlimentar =
      kgAlimentacaoFase > 0 && ganhoTotalKgFase > 0
        ? Number((kgAlimentacaoFase / ganhoTotalKgFase).toFixed(2))
        : null;
