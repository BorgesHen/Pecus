...
import { ModuloSistema, NivelAcesso, PapelUsuario, PermissoesGranulares } from '@pecus/shared';
...
export class UsuariosService {
   ...
   */
  async atualizarInfo(empresaId: string, usuarioId: string, dto: AtualizarUsuarioDto) {
    const vinculo = await this.prisma.usuarioEmpresa.findUnique({
      where: { usuarioId_empresaId: { usuarioId, empresaId } },
    });
    if (!vinculo) throw new NotFoundException('Usuário não vinculado a esta empresa.');
